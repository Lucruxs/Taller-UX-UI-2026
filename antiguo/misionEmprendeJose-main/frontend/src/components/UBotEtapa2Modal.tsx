...
interface UBotEtapa2ModalProps {
  isOpen: boolean;
  onClose: () => void;
  onContinuar: () => void;
  teamColor?: string;
...
export function UBotEtapa2Modal({ 
  isOpen, 
  onClose, 
  onContinuar,
  teamColor 
}: UBotEtapa2ModalProps) {
  ...
  const fullMessage = `¡Etapa 2 Iniciada! 📡\n\nPara crear una solución real, primero deben infiltrarse en la mente del usuario. A esto lo llamamos Protocolo de Empatía.\n\nMisión: Seleccionen un sector. Dentro encontrarán 'Expedientes' de personas con problemas reales. Su trabajo es entender su dolor, no solo venderles algo.\n\n¿Dónde quieren impactar hoy?`;

  // Frases a resaltar (bolding)
  const phrasesToBold = [
    'Etapa 2 Iniciada',
    'Protocolo de Empatía',
    'Misión',
    'Expedientes',
    'entender su dolor',
    '¿Dónde quieren impactar hoy?'
