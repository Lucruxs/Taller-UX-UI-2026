...
export function UpdateGameEtapa2() {
  const navigate = useNavigate();
  const [loading, setLoading] = useState(false);
  const [selectedStage] = useState<number>(2);
  ...
  useEffect(() => {
    ...
    // Cargar actividades de la Etapa 2 autom�ticamente
    const loadActivities = async () => {
      setLoadingActivities(true);
      try {
        const activitiesData = await challengesAPI.getActivities({ stage: 2, include_inactive: 'true' });
              ...
              {loadingActivities ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {(() => {
                    ...
                    if (false) { // Etapa 2 no necesita esta l�gica
                      ...
                      // Para Etapa 2, mostrar todas las actividades normalmente
                      return activities
