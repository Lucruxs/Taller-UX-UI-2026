...
interface UBotBubbleMapModalProps {
  isOpen: boolean;
  onClose: () => void;
  onContinuar: () => void;
  teamColor?: string;
...
export function UBotBubbleMapModal({ 
  isOpen, 
  onClose, 
  onContinuar,
  teamColor 
}: UBotBubbleMapModalProps) {
  ...
  const fullMessage = `¡Mapa de Empatía desplegado! 📡\n\nPara crear una solución real, primero deben conectar profundamente con la persona detrás del problema.\n\nMisión: Analicen la historia del usuario y completen cada espacio del mapa poniéndose en sus zapatos. ¿Qué siente y vive realmente? ¡Completen el esquema para avanzar!`;

  // Frases a resaltar (bolding)
  const phrasesToBold = [
    'Mapa de Empatía desplegado',
    'conectar profundamente',
    'Misión',
    'Analicen la historia del usuario',
    'completen cada espacio del mapa',
    'poniéndose en sus zapatos',
    '¿Qué siente y vive realmente?',
    '¡Completen el esquema para avanzar!'
  ];
