...
interface UBotFormularioPitchModalProps {
  isOpen: boolean;
  onClose: () => void;
  onContinuar: () => void;
  teamColor?: string;
...
export function UBotFormularioPitchModal({ 
  isOpen, 
  onClose, 
  onContinuar,
  teamColor 
}: UBotFormularioPitchModalProps) {
  ...
  const fullMessage = `¡Fase Final Activada! 🎙️🔥\n\nTienen el equipo y tienen el prototipo (LEGO). Ahora toca lo más difícil: Vender la idea.\n\nMisión: Llenen los 5 campos de abajo para crear su guion.\n\n💡 La Fórmula Ganadora:\nConecten el Dolor que descubrieron en la Etapa 2 con la Solución que construyeron en la Etapa 3.`;
